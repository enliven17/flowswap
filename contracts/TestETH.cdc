...
access(all) contract TestETH {
    ...
    access(all) resource Vault {
        ...
        access(all) fun withdraw(amount: UFix64): @TestETH.Vault {
        ...
        access(all) fun deposit(from: @TestETH.Vault) {
    ...
    access(all) fun createEmptyVault(): @TestETH.Vault { return <-create Vault(balance: 0.0) }

    access(all) resource Administrator {
        access(all) fun mintTokens(amount: UFix64): @TestETH.Vault {
            TestETH.totalSupply = TestETH.totalSupply + amount
            emit TokensMinted(amount: amount)
            return <-create Vault(balance: amount)
        }
        access(all) fun burnTokens(from: @TestETH.Vault) {
    ...
    access(all) fun mintTo(recipient: Address, amount: UFix64): @Vault {
        ...
        let receiverRef = recipientAccount.capabilities.get<&TestETH.Vault>(/public/testETHVault)
            .borrow()
    ...
    init() {
        ...
        self.VaultStoragePath = /storage/testETHVault
        self.VaultPublicPath = /public/testETHVault
        self.AdminStoragePath = /storage/testETHAdmin

        let vault <- create Vault(balance: self.totalSupply)
        self.account.storage.save(<-vault, to: self.VaultStoragePath)

        let vaultCap = self.account.capabilities.storage.issue<&TestETH.Vault>(self.VaultStoragePath)
